...
CONFIG_PATH, VERSION = "/data/config.json", "2026.3.36"
...
class Optimizer:
    def __init__(self):
        ...
        self.api_status = {"prices": "offline", "weather": "offline", "ha": "offline"}
    ...
    async def fetch_data(self):
        ...
        if token and entity:
            try:
                async with session.get(f"http://supervisor/core/api/states/{entity}", headers={"Authorization": f"Bearer {token}"}) as r:
                    self.api_status["ha"] = "online" if r.status == 200 else "error"
                    if r.status == 200:
                        data = await r.json()
                        self.current_soc = float(data.get("state", 50.0))
            except: self.api_status["ha"] = "offline"
        ...
        try:
            ...
            async with session.get(url) as r:
                self.api_status["prices"] = "online" if r.status == 200 else "error"
                if r.status == 200:
                    data = await r.json()
        ...
        except: self.api_status["prices"] = "offline"
        ...
        if key and loc:
            try:
                ...
                async with session.get(url) as r:
                    self.api_status["weather"] = "online" if r.status == 200 else "error"
            except: self.api_status["weather"] = "offline"
        else: self.api_status["weather"] = "not_configured"
